...
def benchmark_agent(num_games):
    ...
    # Load the trained agent (Player 2)
    agent = DQNAgent(state_size, action_size)
    try:
        agent.load_model('trained_models/agent2_model.pth')
    ...
    for game in range(1, num_games + 1):
        ...
        while not done and moves < 50:  # Move limit to prevent infinite games
            ...
            if done:
                # Agent (Player 2) wins if their sum reaches target
                p2_sum = sum(state[1])
                if p2_sum >= n:
                    wins += 1
                    if show_details:
