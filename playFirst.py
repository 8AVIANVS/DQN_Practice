...
def play_game():
    ...
    try:
        agent.load_model('trained_models/agent2_model.pth')  # Load agent2's model since AI is playing as Player 2
    ...
    while True:
        ...
        while not game_over:
            ...
            if game_over:
                ...
                p1_sum = sum(state[0])
                p2_sum = sum(state[1])
                if p1_sum >= n:  # You won (Player 1)
                    print("\nYou won!")
                else:  # AI won (Player 2)
                    print("\nAI won!")
